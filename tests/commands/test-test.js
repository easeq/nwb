...
describe('command: test', () => {
  ...
  it('successfully tests a new web module', (done) => {
    cli(['new', 'web-module', 'test-module', '-f'], (err) => {
      ...
      cli(['test'], (cliErr) => {
        expect(cliErr).toNotExist('No errors testing new web module');
        done();
      });
    });
  });

  it('successfully tests a new React component', (done) => {
    cli(['new', 'react-component', 'test-component', '-f'], (err) => {
      ...
      cli(['test'], (cliErr) => {
        expect(cliErr).toNotExist('No errors testing new React component');
        done();
      });
    });
  });

  it('successfully tests a new React app', (done) => {
    cli(['new', 'react-app', 'test-app'], (err) => {
      ...
      cli(['test'], (cliErr) => {
        expect(cliErr).toNotExist('No errors testing new React app');
        done();
      });
    });
  });

  it('successfully tests a new Preact app', (done) => {
    cli(['new', 'preact-app', 'test-app', '-f'], (err) => {
      expect(err).toNotExist('No errors creating new Preact app');
      process.chdir(path.join(tmpDir, 'test-app'));
      cli(['test'], (cliErr) => {
        expect(cliErr).toNotExist('No errors testing new Preact app');
        done();
      });
    });
  });

  it.skip('successfully tests a new Inferno app', (done) => {
    cli(['new', 'inferno-app', 'test-app', '-f'], (err) => {
      expect(err).toNotExist('No errors creating new Inferno app');
      process.chdir(path.join(tmpDir, 'test-app'));
      cli(['test'], (cliErr) => {
        expect(cliErr).toNotExist('No errors testing new Inferno app');
        done();
      });
    });
  });

  it('successfully tests a new web app', (done) => {
    cli(['new', 'web-app', 'test-app'], (err) => {
      expect(err).toNotExist('No errors creating new web app');
      process.chdir(path.join(tmpDir, 'test-app'));
      cli(['test'], (cliErr) => {
        expect(cliErr).toNotExist('No errors testing new web app');
        done();
      });
    });
  });

  it('calls back with an error if tests fail', (done) => {
    cli(['new', 'web-module', 'test-failure-module', '-f'], (err) => {
      ...
      cli(['test'], (cliErr) => {
        expect(cliErr).toExist();
        done();
      });
    });
  });

  it('exits with a non-zero code if tests fail', (done) => {
    cli(['new', 'web-module', 'test-failure-module', '-f'], (err) => {
      expect(err).toNotExist('No errors creating new web module for failure test');
      process.chdir(path.join(tmpDir, 'test-failure-module'));
      const content = fs.readFileSync('./tests/index-test.js', 'utf-8');
