...
function checkDirectories(
  ...
  runSeries(
    dirs.map(dir => taskCb => fs.stat(dir, (err, stats) => {
      if (err) return taskCb(err.code === 'ENOENT' ? null : err);
      taskCb(null, stats.isDirectory() ? dir : null);
    })),
    (err, dirs) => {
...
export function clean(
  ...
  checkDirectories(dirs, (err, dirs) => {
    ...
    runSeries(
      dirs.map(dir => taskCb => fs.remove(dir, taskCb)),
      (err) => {
        if (err) {
          spinner.fail();
          return cb(err);
